...
public class TutorialInventory : MonoBehaviour
{
    ...
    void Update()
    {
        if (tc.stage == 1 && selection.done && !_doneDim)
        {
            // only called once
            _doneDim = true;
        ...
        if (tc.stage == 3 && whichPlayer == 1 && (Keyboard.current.leftShiftKey.wasPressedThisFrame || Input.GetButtonDown("B1")))
        {
        ...
        if (tc.stage == 3 && whichPlayer == 2 && (Keyboard.current.rightShiftKey.wasPressedThisFrame || Input.GetButtonDown("B2")))
        {
            // if (BlockController) BlockController.UnSelectBlock(cards[_selectedIndex].index);
            // RotateIndex();
            // if (BlockController) BlockController.SelectBlock(cards[_selectedIndex].index);
